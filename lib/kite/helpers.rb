module Kite::Helpers
  ...
  def check_cloud_config(config)
    raise Kite::Error::CloudConfigInvalid unless config.find { |key, hash| hash.find { |k, v| v.nil? } }.nil?
  end

  # Check if Terraform IaC was applied
  def check_terraform_applied
    raise Kite::Error::TerraformNotApplied unless File.file? "terraform/terraform.tfstate"
  end
